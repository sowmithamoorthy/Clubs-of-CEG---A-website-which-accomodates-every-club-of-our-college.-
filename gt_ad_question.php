...
<head>
    ...
    <title>Admin Page - GT Leadership Club</title>
    <style>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['answer'], $_POST['question_id'], $_POST['submit_answer'])) {
    ...
    // Fetch the question from the 'gt_con' table
    $fetchQuestionQuery = "SELECT name, email, queries FROM gt_con WHERE id = $questionId";
    ...
    $question = $questionRow['queries'];

    // Insert the question, answer, and email into the 'answers' table
    $insertQuery = "INSERT INTO gt_answer (email, question, answer) VALUES ('$email', '$question', '$answer')";
    ...
    if (!$insertResult) {
        ...
        $deleteQuery = "DELETE FROM gt_con WHERE id = $questionId";
...
if (pg_num_rows($result) > 0) {
    while ($row = pg_fetch_assoc($result)) {
        ...
        $question = isset($row['queries']) ? $row['queries'] : '';
