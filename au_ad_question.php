...
<head>
    ...
    <title>Admin Page - aupodium speech club</title>
    <style>
        body {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['answer'], $_POST['question_id'], $_POST['submit_answer'])) {
    ...
    // Fetch the question from the 'theatron_con' table
    $fetchQuestionQuery = "SELECT name, email, question FROM aupodium_con WHERE id = $questionId";
    $fetchQuestionResult = pg_query($dbconn, $fetchQuestionQuery);

    if (!$fetchQuestionResult) {
        die("Failed to fetch question. Error: " . pg_last_error($dbconn));
    }
    ...
    // Insert the question, answer, and email into the 'theatron_answer' table
    $insertQuery = "INSERT INTO aupodium_answer (email, question, answer) VALUES ('$email', '$question', '$answer')";
    ...
    if (!$insertResult) {
        ...
        // Delete the question from the 'theatron_con' table
        $deleteQuery = "DELETE FROM aupodium_con WHERE id = $questionId";
...
// Fetch queries from the theatron_con table
$query = "SELECT * FROM aupodium_con";
