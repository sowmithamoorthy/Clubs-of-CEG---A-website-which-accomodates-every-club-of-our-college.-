...
<head>
    ...
    <title>Admin Page - CTF Leadership Club</title>
    <style>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['answer'], $_POST['question_id'], $_POST['submit_answer'])) {
    ...
    // Fetch the question from the 'ctf_con' table
    $fetchQuestionQuery = "SELECT name, email, question FROM ctf_con WHERE id = $questionId";
    ...
    // Insert the question, answer, and email into the 'ctf_answer' table
    $insertQuery = "INSERT INTO ctf_answer (email, question, answer) VALUES ('$email', '$question', '$answer')";
    $insertResult = pg_query($dbconn, $insertQuery);

    if (!$insertResult) {
        echo "Failed to insert answer. Error: " . pg_last_error($dbconn);
    } else {
        // Delete the question from the 'ctf_con' table
        $deleteQuery = "DELETE FROM ctf_con WHERE id = $questionId";
...
// Fetch queries from the ctf_con table
$query = "SELECT * FROM ctf_con";
